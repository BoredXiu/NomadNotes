...
export async function createExpense(
  ...
  const res = await api.post<ApiResponse<Expense>>(
...
export async function getTripExpenses(
  ...
  const res = await api.get<ApiResponse<PaginatedData<Expense>>>(
    `/trips/${tripId}/expenses`,
    { params },
  );
  return res.data.data;
...
export async function getExpenseStats(
  ...
  const res = await api.get<ApiResponse<ExpenseStats>>(
...
export async function uploadFileToTemp(
  ...
  const res = await api.post<ApiResponse<TempFileResult>>(
    "/upload",
    formData,
    {
...
export async function persistSingle(
  fileId: string,
  ext: string,
): Promise<{ imageUrl: string }> {
  const res = await api.post<ApiResponse<{ imageUrl: string }>>(
    "/upload/persist-single",
    { fileId, ext },
  );
  return res.data.data;
}

export async function persistMultiple(
  files: { fileId: string; ext: string }[],
): Promise<{ imageUrl: string }[]> {
  const res = await api.post<ApiResponse<{ imageUrl: string }[]>>(
    "/upload/persist-multiple",
    { files },
  );
  return res.data.data;
}