...
export function usePWAInstall() {
	...
	const [checkingInstall, setCheckingInstall] = useState(true); // 初始化检测完成前为 true
	...
	const install = useCallback(async () => {
		...
		if (deferredPrompt) {
			...
			if (result.outcome === "accepted") {
				setCanInstall(false);
				setDeferredPrompt(null);
				message.success("应用已成功添加到桌面");
			} else {
				message.info("已取消安装");
			}
		} else {
			// 先检查是否已通过 getInstalledRelatedApps 检测到已安装（Chrome 80+）
			try {
				if ("getInstalledRelatedApps" in navigator) {
					const relatedApps = await (navigator as any).getInstalledRelatedApps();
					if (relatedApps.some((app: any) => app.platform === "webapp")) {
						setCanInstall(false);
						message.info("应用已安装到设备上，请在已安装的应用中打开");
						return;
					}
				}
			} catch {
